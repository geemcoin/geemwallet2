...
const char DEFAULT_WALLET_FILE_NAME[] = "geem.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultNodeList << "seed5.geem.io:2046";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("geemwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("geemwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("geemwallet.desktop");
  ...
  res = autorunSettings.contains("geemwallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("geemwallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.geem.geemwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Geem Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("geemwallet", appPath);
    } else {
      autorunSettings.remove("geemwallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\Geem", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:geem");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\Geem\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\Geem\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("geemwallet.cfg"));
