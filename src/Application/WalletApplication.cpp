...
const char URI_SCHEME_NAME[] = "geem";
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("geemwallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] Geem wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "Geem wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have geemd with non-default RPC port, you should terminate it and relaunch GeemWallet\n"
      "or\n"
      "Set the Local deamon required port in GeemWallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
